...
시계열 데이터 시리즈의 첫번째로 기본적인 시계열 데이터의 특징을 알아보고 간단한 모델을 만들어 보았습니다.  
이에 이어 시리즈의 두번째 내용으로 tf.data.Dataset의 전체적인 설명과 이를 이용한 데이터 구성을 해보겠습니다. <!--more-->
...
![orod](/assets/img/posts/time_series_2/orod.png)  

딱 봐도 같은 데이터가 여러번 반복되고있고 메모리 사용이 비효율 적일 것이라는게 눈에 보입니다. 그리고 이를 만드는 과정 자체도 수고롭습니다.  
tf.data를 이용해 이 모든 것을 해결할 수 있습니다. 메모리를 효율적으로 사용하고 만드는 과정도 간편하게 만들 수 있습니다.

## tf.data

TensorFlow 공식 document를 읽어보면 tf.data에 대한 설명이 다음과 같이 나와있습니다.
...
간단하게 요약하자면 입력 파이프라인 빌드를 쉽게, 빠르게 할 수 있다는 내용입니다.  
한번에 모든 데이터를 구성하여 메모리에 올려놔야하는 feed_dict 방식에 비해 tf.data는 dataset 자체에서 batch와 shuffle, cache, prefetch 등을 지원하기 때문에 훨씬 빠르고 효율적입니다.

### How to use

코드를 보고 기능을 하나씩 짚어보겠습니다.  
아래 데이터를 사용합니다. (지난번 포스팅의 데이터와 같습니다.)  

![data](/assets/img/posts/time_series_2/data.png)  
...
가장 바깥쪽 dim을 이용해 데이터를 잘라 dataset의 dim으로 활용합니다.
...
처음 `tf.constant`로 만든 데이터인 `a`는 출력하면 데이터의 세부 내용이 전부 출력되지만 이를 `from_tensor_slices`를 이용해 변환한 `a_ds`는 data shape, type만 출력될 뿐 세부 내용이 출력되지 않습니다.  
이의 세부 내용을 보기 위해서는 위와 같이 for문을 이용해 출력하거나 `as_numpy_iterator`등 을 이용하여 출력합니다. (이 외에도 여러 방법들을 사용할 수 있습니다.)

모든 tf.data는 이런 특징을 가집니다.

#### window

`window`의 기능은 다음 예시로 쉽게 이해가 됩니다.
...
`window`는 이렇게 시계열 데이터를 구성할 때 유용합니다.  
`size`와 `shift`는 시계열 첫번째 시리즈 포스팅에서 말한 **window_size**와 **shift**를 의미합니다.  
`drop_reaminder`는 다음 예시로 이해 가능합니다. `drop_remainder=False`로 주게 되면 결과는 다음과 같아집니다.
...
그런데 `a_window`를 출력함에 있어 이상한 점이 하나 보입니다. for문을 사용했음에도 그 안에 `as_numpy_iterator`를 또 사용했습니다.  
for문 만을 사용하면 출력 결과는 다음과 같이 나옵니다.
...
이를 보면 알 수 있듯이 window로 분할된 sub_dataset들도 tf.data의 특성을 가집니다.  
다른 방법으로도 출력해보겠습니다.
...
이렇게 예상과 다른 출력 결과가 나오는 이유는 `a_ds`가 이렇게 생겼기 때문입니다.
...
이렇게 tf.Tensor들을 5개씩 묶은것 뿐이기 때문입니다. 요컨데 이런 형태로 만든거죠
...
즉 이를 데이터 셋으로 활용하기 위해서는 **바로 위와 같이 생긴 녀석들을 `tf.Tensor([0 1 2 3 4], shape=(5,), dtype=int32)` 이런 형태로 차원을 줄이고 flatten해주는 과정**이 필요합니다.  
이를 위해 `flat_map`과 `batch`를 사용합니다.

#### flat_map

기본적으로는 흔히 사용하는 파이썬 기본 기능 `map`과 같은 기능을 합니다.  
하지만 그에 더해 차원을 하나 축소시킵니다.
...
**map**  
map도 있습니다. 위 코드에서 `flat_map`을 `map`으로 바꾸면 다음과 같습니다.
...
이를 `window_size`만큼 잘라 하나의 tf.Tensor 안에 묶어주기 위해 batch를 사용합니다.
...
이렇게 window를 거친 데이터를 원하는 형태로 flatten 해주기 위해 `flat_map`과 `batch`를 통한 일종의 트릭을 사용했고, 성공적으로 마쳤습니다.  
(개인적으로는 특정 차원을 직접 flatten할 수 있는 기능이 추가되면 좋겠습니다. 그러면 이렇게 복잡한 과정을 거치지 않아도 될 것 같아요... 언젠가는 추가되길 바랍니다.)

#### zip

`zip`은 이름 그대로 짝지어 묶어주는 기능을 합니다.
...
`shuffle`은 말 그대로 데이터를 섞어주는 기능을 합니다.  
근데 `ds.shuffle(512)`이와 같이 parameter를 하나 받습니다. 단지 섞어주는 것뿐인데 인수를 왜 받는지, 무슨 역할을 하는지 의문이 생깁니다.

이 인수는 `buffer_size`입니다.
이 인수의 기능을 알기에 앞서 tf.data를 사용하는 이유중 큰 부분은 효율적인 메모리 사용에 있습니다.  
대용량 데이터를 처리할 때, 극단적으로 데이터 용량이 RAM 용량보다 큰 경우라면 shuffle을 하기 위해 모든 데이터를 전부 가져와 shuffle 하는 것은 당연히 불가능합니다.  
쉽게 이해하자면 `buffer_size`는 이를 위해 해당 사이즈만큼의 데이터만을 가져와서 shuffle을 해주는 것이라고 생각하면 됩니다.  

위와 같이 `buffer_size`를 512로 설정하면 훈련 과정에서는 다음과 같은 과정이 반복됩니다.
...
`cache` 역시 이름 그대로 cache형태로 데이터를 저장합니다.  
훈련시 첫번째 epoch은 데이터를 가져오는 과정이 포함되기 때문에 다른 epoch에 비해 느립니다. cache를 사용하면 두번째 epoch부터 이 과정을 거치지 않고 캐시 데이터를 사용합니다.

#### prefetch

`prefetch`도 위의 것들과 마찬가지로 데이터를 미리(pre)+가져(fetch)옵니다.  
훈련시에 특정 데이터를 필요로 하는 순간 그 데이터를 가져오기 시작한다면 훈련하는 과정은 그 사이에 놀게 되겠죠. 이를 방지해주기 위해서 사용합니다.  
데이터가 필요한 순간에 해당 데이터가 미리 준비되어 있도록 병렬적으로 데이터를 가져옵니다.
...
이를 위한 과정 외에는 위에서 사용했던 기능 그대로 사용하면 됩니다.
...
위 과정을 통해 tf.data.Dataset을 만들어 보았습니다. 이를 이용한 train 및 evaluate 과정은 전체 원본 코드에서 확인하실 수 있습니다.

현재까지는 target column만을 이용한 시계열 데이터 구성과 모델링을 해보았습니다.  
다음 포스팅에서는 target column 뿐만 아니라 다른 데이터까지 함께 사용한 시계열 데이터를 다뤄보겠습니다.
