...
유튜브 **빵형의 개발도상국** 채널의 **[내 얼굴로 어노잉 오렌지 만들기](https://www.youtube.com/watch?v=9VYUXchrMcM&t=204s)** 영상을 따라하되 약간의 변형과 추가만 했습니다.  
좋은 강의 감사합니다!
...
모델은 따로 학습하지 않고 기존에 만들어져있는 모델을 사용합니다.  
[이곳](https://github.com/davisking/dlib-models)에서 다운로드 가능합니다. 이 중 사용할 모델은 `shape_predictor_68_face_landmarks`이며 `dlib`을 이용해 사용합니다.  

![face_landmarks](/assets/img/posts/why55_chimrangeee/face_landmarks.png)

위 사진은 모델이 검출하는 얼굴의 landmark를 표시한 것입니다.  
모델을 사용하면 0~67번까지 총 68개의 landmark들을 검출하게 되며 사진 내의 픽셀 위치 값들을 리턴해줍니다.
...
아 ㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋ 너무 만족스럽습니다 ㅋㅋㅋㅋㅋㅋㅋ

침착맨의 팬으로서는 정말 웃음벨인데 ㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋ  
모르시는 분들도 재미있게 볼지는 모르겠네요.

## 보완점

### 합성된 눈과 입이 흔들린다.

사실 흔들리는것 때문에 B급 감성이 더해져 더 웃긴것 같아 당장은 만족스럽긴 합니다만 ㅋㅋㅋㅋㅋㅋㅋ  
그래도 결과물의 퀄리티를 놓고 보면 확실히 아쉬운 부분입니다.

오렌지에 위치시킬 눈과 입의 픽셀위치는 fix 되어있기 때문에 흔들리는 원인은 crop 하면서 눈 크기와 입 크기에 따라 crop 되는 이미지 자체의 크기가 달라지기 때문이라고 볼 수 있을 것 같습니다.  
크기뿐만 아니라 얼굴을 움직이면서 말하기 때문에 이도 영향을 미치는 것으로 보입니다.

### 얼굴이 움직이는 방향대로 오렌지가 움직이지 않는다.

얼굴 움직이는대로 움직이지 않는 것 또한 B급 감성인듯 하여 만족스럽지만 위에서 말한 것 처럼 이를 해결하면 합성된 눈과 입이 흔들리는 현상도 어느정도 커버가 가능할 것으로 보입니다.  
완벽하게 똑같이 움직일 수는 없겠지만 비슷하게 움직이게 하는 방법은 간단하다 생각합니다.  
원본 얼굴 이미지상 두 눈과 입의 위치를 상대값으로 받아 오렌지의 상대값에 그대로 넣어주면 쉽게 해결될 듯 합니다.

### 손이나 자막이 순간적으로 얼굴을 가리면 오렌지에 합성되는 얼굴 또한 순간적으로 사라진다.

프레임 자체를 결측값으로 바라본다면, 손이 얼굴을 가리기 직전과 직후의 사진으로 적당한 interpolation을 통해 가장 간단하게 해결할 수 있을 것으로 보입니다.  
다만 하나의 프레임안에서 정확히 가려진 부분만 어떤 형태인지 알아내기 위해서는 좀 더 복잡한 과정이 필요하겠네요.

### 눈, 입을 crop하는 로직이 이미지가 뒤집히면 제대로 crop을 제대로 수행할지 의문이다.

눈과 입을 잘라내는 로직을 보면 얼굴을 똑바로 봤을 때 위, 아래, 왼쪽, 오른쪽의 가장 바깥쪽 landmark를 가져와 잘라냅니다.  
만약 뒤집힌 이미지나 영상을 사용한다면 위, 아래, 왼쪽, 오른쪽이 함께 뒤집힐 것인데 이에 대한 대비가 필요할 것입니다.

### face landmark 모델이 어떤 원리로 작동하는지 알지 못한다.

추가적인 공부가 필요한 부분입니다.  
사실 오늘의 포스팅은 머신러닝에 대한 공부라기보단 `cv2`, `dlib`, `moviepy` 등을 이렇게 활용할 수 있다! 정도였습니다.  

기회가 된다면 face landmark 모델은 어떤 방식으로 학습되었는지, 원리가 무엇인지 등을 알아보겠습니다.

이런 재미 위주의 글도 종종 업로드 예정입니다.
