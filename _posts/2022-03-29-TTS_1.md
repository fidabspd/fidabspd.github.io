...
우선은 논문에서 제안하는 아키텍쳐 기존의 기본적인 Transformer 아키텍쳐와 비교하여 살펴보자.  
...
하지만 기본적인 Transformer의 뼈대는 일치하지만 조금 다른 부분이 몇가지 보인다.  
나열해보자면 다음과 같다.
...
하지만 정말 감사하게도 현재 사용하는 스크립트에는 '서른' 뿐만 아니라 '하나', '둘' 처럼 기수가 아닌 서수 형태로 숫자를 읽는 경우에는 발음 그대로 한글로 표현해두었다.  
따라서 숫자는 모두 기수 형태로 처리해주었다.
