...
input으로 활용할 수 있는 정보는 앞선 시간대의 시계열 정보 뿐만이 아닙니다. target time의 정보 또한 시계열 데이터 예측에 중요한 역할을 합니다.  
...
위 데이터의 `date_time`은 있는 그대로 사용할 수 없습니다. 이를 가공해주기 위해 다음 과정을 거칩니다.
...
![new_data](/assets/img/posts/time_series_3/new_data.png)

### Scaling

standard scaling을 진행하며 `inversed_rmse`를 metric으로 활용해주기 위해 `mean`과 `std`는 따로 저장합니다.  
...
(사실 EDA를 위한 탐색이 선행되어야 합니다. 본 포스팅 시리즈에서는 이 과정은 생략합니다.)

현재 `target_length: 3`이기 때문에 맞추고자 하는 3개의 시간 중 가운데 정보만을 활용하도록 하겠습니다.  
(이를 그대로 input으로 넣어준다던지, 평균을 이용한다던지 하는 방법도 당연히 가능합니다.)
...
모델 구조를 만들기에 앞서, model metric으로 사용할 inverse된 target을 이용한 `inversed_rmse`를 계산하는 함수를 만들어줍니다.
...
이를 활용하는 방법은 정말 다양합니다.  
이중 옵션을 통해 선택할 수 있는 5가지 선택지를 만들도록 하겠습니다.  
(5가지 외에도 다양한 방법들이 있지만 전부 다룰 수는 없기 때문에 가장 기본적인 내용들로만 구성했습니다.)
...
(각 모델 구조에 대한 자세한 설명은 생략합니다.)  

각 모델들의 마지막을 flatten하여 `target time`정보와 `Concatenate`하여 `Dense`레이어를 활용하여 최종 output을 뽑습니다.
...
![model_summary](/assets/img/posts/time_series_3/model_summary.png)
...
현재까지는 `num == 1`인 데이터만을 사용하였습니다.  
다음 시계열 포스팅 시리즈에서는 60개의 건물을 모두 하나의 모델의 데이터로 활용하는 multi-task learning에 대해 다뤄보겠습니다.
